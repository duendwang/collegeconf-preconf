<?php
/**
 * @var \App\View\AppView $this
 * @var \App\Model\Entity\ConferenceLocality[]|\Cake\Collection\CollectionInterface $conferenceLocalities
 */
?>
<div class="conferenceLocalities index content">
    <?= $this->Html->link(__('New Conference Locality'), ['action' => 'add'], ['class' => 'button float-right']) ?>
    <h3><?= __('Conference Localities') ?></h3>
    <div class="table-responsive">
        <table>
            <thead>
                <tr>
                    <th><?= $this->Paginator->sort('id') ?></th>
                    <th><?= $this->Paginator->sort('conference_id') ?></th>
                    <th><?= $this->Paginator->sort('locality_id') ?></th>
                    <th><?= $this->Paginator->sort('user_id') ?></th>
                    <th><?= $this->Paginator->sort('registering') ?></th>
                    <th><?= $this->Paginator->sort('creator') ?></th>
                    <th><?= $this->Paginator->sort('created') ?></th>
                    <th><?= $this->Paginator->sort('modifier') ?></th>
                    <th><?= $this->Paginator->sort('modified') ?></th>
                    <th class="actions"><?= __('Actions') ?></th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($conferenceLocalities as $conferenceLocality): ?>
                <tr>
                    <td><?= $this->Number->format($conferenceLocality->id) ?></td>
                    <td><?= $conferenceLocality->has('conference') ? $this->Html->link($conferenceLocality->conference->id, ['controller' => 'Conferences', 'action' => 'view', $conferenceLocality->conference->id]) : '' ?></td>
                    <td><?= $conferenceLocality->has('locality') ? $this->Html->link($conferenceLocality->locality->name, ['controller' => 'Localities', 'action' => 'view', $conferenceLocality->locality->id]) : '' ?></td>
                    <td><?= $conferenceLocality->has('user') ? $this->Html->link($conferenceLocality->user->id, ['controller' => 'Users', 'action' => 'view', $conferenceLocality->user->id]) : '' ?></td>
                    <td><?= h($conferenceLocality->registering) ?></td>
                    <td><?= $this->Number->format($conferenceLocality->creator) ?></td>
                    <td><?= h($conferenceLocality->created) ?></td>
                    <td><?= $this->Number->format($conferenceLocality->modifier) ?></td>
                    <td><?= h($conferenceLocality->modified) ?></td>
                    <td class="actions">
                        <?= $this->Html->link(__('View'), ['action' => 'view', $conferenceLocality->id]) ?>
                        <?= $this->Html->link(__('Edit'), ['action' => 'edit', $conferenceLocality->id]) ?>
                        <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $conferenceLocality->id], ['confirm' => __('Are you sure you want to delete # {0}?', $conferenceLocality->id)]) ?>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
    </div>
    <div class="paginator">
        <ul class="pagination">
            <?= $this->Paginator->first('<< ' . __('first')) ?>
            <?= $this->Paginator->prev('< ' . __('previous')) ?>
            <?= $this->Paginator->numbers() ?>
            <?= $this->Paginator->next(__('next') . ' >') ?>
            <?= $this->Paginator->last(__('last') . ' >>') ?>
        </ul>
        <p><?= $this->Paginator->counter(__('Page {{page}} of {{pages}}, showing {{current}} record(s) out of {{count}} total')) ?></p>
    </div>
</div>
